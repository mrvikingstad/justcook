<script lang="ts">
	...
	let servings = $state<number | null>(4);
	let photoFile = $state<File | null>(null);
	let photoPreview = $state<string | null>(null);

	// Language state
	let language = $state(data.userLanguageCode || 'en');
	let showOtherLanguage = $state(false);
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			if (!response.ok) {
				error = result.error || 'Failed to publish recipe';
			} else {
...
<svelte:head>
	<title>New Recipe - Just Cook</title>
</svelte:head>
...
<div class="page container">
	<header class="page-header">
		...
		<div>
			<h1>New Recipe</h1>
			<p class="subtitle">Share your creation with the world.</p>
		</div>
	</header>
	...
	<form onsubmit={handleSubmit}>
		...
		<section class="section submit-section">
			...
			<p class="submit-hint">You can edit your recipe anytime after publishing.</p>
		</section>
	</form>
</div>
